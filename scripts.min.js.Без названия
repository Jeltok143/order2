...
$(function () {
    ...
    $('.select-custom').each(function () {
        ...
        if ($this.children('option:selected').val() === '') {
            $styledSelect.addClass('select-unselected');
        }
        ...
        $listItems.click(function (e) {
            ...
            $styledSelect.removeClass('select-unselected');
            $list.hide();
        });
        ...
        $(this).keypress(function (event) {
            if (event.keyCode == 13) {
                ...
                $this.next('div.select-styled').click();
            }
        });
    ...
    $('.select-options, textarea').niceScroll({
        cursorcolor: "#d50032",
        cursorborder: "none",
        cursoropacitymin: 1,
        cursorwidth: "3px",
    ...
    // CALCULATION VALISE - TABS
    $('.valise__tabs button').click(function (e) {
        if ($(this).hasClass('active')) {
            return false;
        }

        $('.valise__tabs button.active').removeClass('active');
        $(this).addClass('active');
        $('.box-valise-hide.active').removeClass('active');
        $('#' + $(this).attr('data-id')).addClass('active');
    });


    // CALCULATION VALISE - COLOR PICKERS
    $('.valise__wrap__calculation .row-calc ul li, .box__logo__valise li').click(function (e) {
        if ($(this).hasClass('active')) {
            return false;
        }

        var par = $(this).closest('ul');
        var selected = par.siblings('.title').find('span');

        selected.text($(this).find('img').attr('title')); //set selected color in title
        ...
        //new mod
        //new mod
        if (par.hasClass('valise__calculation__color__1')) { // оконтовка
            var okon = $(this).attr('data-okon');
            $('.valise-images__layer_border img').attr('src', okon);
        }

        if (par.hasClass('valise__calculation__color__2')) { // оконтовка
            var nitka = $(this).attr('data-okon');
            $('.valise-images__layer_thread img').attr('src', nitka);
        }

        if (par.hasClass('valise__calculation__color__3')) { // оконтовка
            var nitka = $(this).attr('data-okon');
            $('.valise-images__layer_material img').attr('src', nitka);
        }

        //logo on valise
        $('.valise-images__layer_logo img').remove(); // remove logo
        if (par.hasClass('box__logo__valise__ul')) { // logo
            var price_text = $('.valise__price_current span');
            var price = parseInt($('.valise__price_current').data('price'));
            var price_logo = parseInt($('.valise-logo__price').data('price'));

            if ($(this).hasClass('first')) {
                price_text.text(price); //reset cost
            } else {
                price_text.text(price + price_logo); //set cost with logo
                var img = $(this).find('img').attr('src');
                $('.valise-images__layer_logo').append('<img class="img-fluid" src="' + img + '">');
            }
        }
    ...
    //VALISE ORDER
    $("a.valise__order").click(function(e) {
        e.preventDefault();

        const form   = $('#form-order');

        //add title
        form.find('.form-total').remove();
        form.find('h3').text('Заказать "Автомобильный саквояж"');

        //add goal for YM
        form.find('form').attr('onsubmit', "ym(50828787, 'reachGoal', 'sakvo-zakaz'); return true;");

        //add params in form
        const border = $('.valise__calculation__color__1 li.active img');
        const thread = $('.valise__calculation__color__2 li.active img');
        const material = $('.valise__calculation__color__3 li.active img');
        const elLogo = $('.box__logo__valise__ul li.active img');

        form.find('input[name=valise1]').val(border.attr('src')+';'+border.attr("alt"));
        form.find('input[name=valise2]').val(thread.attr('src')+';'+thread.attr("alt"));
        form.find('input[name=valise3]').val(material.attr('src')+';'+material.attr("alt"));
        if (elLogo.length > 0){
            form.find('input[name=valise_logo]').val(elLogo.attr('src')+';'+elLogo.attr("alt"));
        }
    ...
    //VALISE SLIDERS FOR ADAPTIVE
    $('.valise__photos').slick({
        adaptiveHeight: true,
        dots: false,
        infinite: true,
        speed: 300,
